...
class mouse_plotObj:
    def __init__(self, ID, position, params = {}):
        ...
        self.curr_rem_dur = 0
        self.delay = int(np.round(120/2.5))
        self.delay_count = 0 
        self.dur_count = 0
        self.on_delay = False
        self.laser_on = False
        ...
        if len(mouseObj.EEG_5) >= 5000:
            ...
            if mouseObj.prem == 1:
                ...
                if ((th_delta > th_thl) and (pow_mu < th_m)):
                    ...
                    mouseObj.prem = 0
                    mouseObj.dur_count += mouseObj.curr_rem_dur
                    mouseObj.delay_count = mouseObj.delay
                    mouseObj.curr_rem_dur = 0
                    mouseObj.on_delay = True
                    # self.signal_clMode.emit(False, mouseObj.position, 0)

            if mouseObj.on_delay:
                if mouseObj.prem == 0:
                    mouseObj.delay_count -= 1

                    if mouseObj.delay_coun == 0:
                        self.signal_clMode.emit(True, mouseObj.position, 0)
                        mouseObj.laser_on = True
                        mouseObj.on_delay = False

            if mouseObj.laser_on:
                if mouseObj.prem == 0:
                    if mouseObj.dur_count >= 0:
                        mouseObj.dur_count -= 1
                    else:
                        self.signal_clMode.emit(False, mouseObj.position, 0)
                        mouseObj.laser_on = False
                else:
                    self.signal_clMode.emit(False, mouseObj.position, 0)
                    mouseObj.laser_on = False
